...
export function setTranslateCookie(lang) {
  ...
  const mrVal = isOriginal ? '' : `/mr/${lang}`;
  const autoVal = isOriginal ? '' : `/auto/${lang}`;
  const expires = isOriginal 
    ? 'expires=Thu, 01 Jan 1970 00:00:00 UTC;' 
    : 'expires=Fri, 31 Dec 2030 23:59:59 GMT;';

  // Path /
  document.cookie = `googtrans=${mrVal}; ${expires} path=/;`;
  document.cookie = `googtrans=${mrVal}; ${expires} path=/; domain=${host};`;
  document.cookie = `googtrans=${autoVal}; ${expires} path=/;`;
  document.cookie = `googtrans=${autoVal}; ${expires} path=/; domain=${host};`;

  // Domain variants for subdomains / naked domain
  const hostParts = host.split('.');
  if (hostParts.length > 1) {
    const rootDomain = '.' + hostParts.slice(-2).join('.');
    document.cookie = `googtrans=${mrVal}; ${expires} path=/; domain=${rootDomain};`;
    document.cookie = `googtrans=${autoVal}; ${expires} path=/; domain=${rootDomain};`;
  }
}
...
export function triggerGoogleTranslateCombo(lang) {
  ...
  if (select) {
    const val = lang === 'mr' ? '' : lang;
    select.value = val;
    
    // Dispatch bubbling change and input events
