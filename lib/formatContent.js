export function renderRichContent(text) {
  ...
  html = html.replace(/\[(.*?)\]\((.*?)\)/g, (match, linkText, url) => {
    ...
    linkPlaceholders.push(
      `<a href="${cleanUrl}" target="_blank" rel="noopener noreferrer" style="color: #003884; text-decoration: underline; font-weight: 600; word-break: break-all;">${linkText}</a>`
    );
    return `__LINK_PH_${idx}__`;
  });

  // 3. Process standalone raw URLs (http://, https://, or www.)
  html = html.replace(/(https?:\/\/[^\s<>'"]+|www\.[^\s<>'"]+)/gi, (match) => {
    ...
    return `<a href="${href}" target="_blank" rel="noopener noreferrer" style="color: #003884; text-decoration: underline; font-weight: 600; word-break: break-all;">${url}</a>${trailingPunct}`;
  });
