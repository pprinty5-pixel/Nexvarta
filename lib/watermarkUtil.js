    ...
    img.onload = () => {
      try {
        ...
        if (position === 'bottom-banner') {
          ...
          // Clean Badge Text (single clean bullet, strip leading emojis or existing symbols)
          const rawTag = watermarkText.replace(/^[🔴⚡📍❌●◆\s]+/, '').trim();
          const cleanBadgeText = `● ${rawTag || 'NEXVARTA'}`;
          ...
          if (showLocation && locationText && locationText.trim()) {
            ...
            ctx.textAlign = 'left';
            ctx.textBaseline = 'middle';
            ctx.fillText(locationText.trim(), pillX + pillWidth + 14, pillY + pillHeight / 2);
          }

          // Optional Right Brand Digital Seal (NVNEWS.IN)
          if (showDomain && domainText && domainText.trim()) {
            ...
            ctx.textAlign = 'right';
            ctx.textBaseline = 'middle';
            ctx.fillText(domainText.trim().toUpperCase(), width - 24, pillY + pillHeight / 2);
          }
