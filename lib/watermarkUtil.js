    ...
    img.onload = () => {
      try {
        ...
        if (position === 'bottom-banner') {
          ...
          const pillWidth = Math.min(220, Math.round(width * 0.28));
          ...
          ctx.font = `bold ${Math.round(pillHeight * 0.52)}px 'Segoe UI', system-ui, sans-serif`;
          ctx.textAlign = 'center';
          ctx.textBaseline = 'middle';
          ctx.fillText(watermarkText.replace(/^🔴\s*/, '● '), pillX + pillWidth / 2, pillY + pillHeight / 2);

          // Location and Date Subtitle
          ctx.fillStyle = '#f8fafc';
          ctx.font = `600 ${Math.round(bannerHeight * 0.28)}px 'Segoe UI', system-ui, sans-serif`;
          ctx.textAlign = 'left';
          ctx.fillText(locationText, pillX + pillWidth + 14, pillY + pillHeight / 2);

          // Right Nexvarta Digital Seal
          ctx.fillStyle = 'rgba(255, 255, 255, 0.85)';
          ctx.font = `800 ${Math.round(bannerHeight * 0.32)}px 'Segoe UI', system-ui, sans-serif`;
          ctx.textAlign = 'right';
